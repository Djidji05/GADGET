<template>
  <div>
    <h2 class="text-lg font-semibold text-gray-800 mb-6">Météo & Pratique</h2>
      ...
      <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <div class="flex flex-col sm:flex-row gap-6 mb-6">
            <div class="flex items-center">
                ...
                >
                <label for="isActiveSection" class="ml-2 block text-sm text-gray-900">Afficher cette section</label>
            </div>

            <div class="flex items-center gap-2">
                <span class="text-sm font-medium text-gray-700">Mode :</span>
                <select v-model="config.content.mode" class="text-sm border-gray-300 rounded-lg focus:ring-blue-500">
                    <option value="manual">Manuel (Liste fixe)</option>
                    <option value="automatic">Automatique (Contexte/Tags)</option>
                </select>
            </div>

            <div v-if="config.content.mode === 'automatic'" class="flex items-center gap-2">
                <span class="text-sm font-medium text-gray-700">Forcer la météo :</span>
                <select v-model="config.content.weatherOverride" class="text-sm border-gray-300 rounded-lg focus:ring-blue-500 bg-blue-50 font-bold text-blue-700">
                    <option value="">Auto (Basé sur date)</option>
                    <option value="noel">Noël / Fêtes</option>
                    <option value="rain">Pluvieux</option>
                    <option value="storm">Tempête / Vent</option>
                    <option value="summer">Chaleur / Été</option>
                    <option value="school">Rentrée scolaire</option>
                </select>
            </div>
        </div>
        
        <div v-if="config.content.mode === 'automatic'" class="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-start gap-3">
          <i class="fas fa-info-circle text-blue-600 mt-1"></i>
          <p class="text-sm text-blue-600">
            En mode <b>Automatique</b>, le système affichera les cartes correspondant au contexte actuel (ex: Fêtes en Décembre). 
            Utilisez les <b>Tags</b> sur chaque carte pour définir quand elles doivent apparaître.
          </p>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <div class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 bg-white/90 rounded-lg shadow-sm p-1">
                 ...
                 <button @click="removeCard(index)" class="p-1 hover:text-red-600"><i class="fas fa-trash"></i></button>
              </div>

              <!-- HEADER -->
              <div class="mb-4">
                <div class="flex justify-between items-start mb-1">
                    <h4 class="font-bold text-lg text-gray-900 truncate flex-1" :title="card.title">{{ card.title || 'Titre de la carte' }}</h4>
                    <!-- Tag Badge -->
                    <span 
                      v-if="card.tag" 
                      class="ml-2 px-1.5 py-0.5 bg-blue-100 text-blue-700 text-[10px] font-bold rounded uppercase tracking-wide border border-blue-200 shadow-sm"
                    >
                      #{{ card.tag }}
                    </span>
                </div>
      ...
      <div v-if="showModal" class="fixed inset-0 z-[200000] flex items-center justify-center p-4 bg-gray-100">
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] relative z-[200001]">
          <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h3 class="text-lg font-bold text-gray-900">{{ editingIndex !== null ? 'Modifier la carte' : 'Nouvelle carte' }}</h3>
          ...
          <div class="p-6 overflow-y-auto custom-scrollbar flex-1 min-h-0">
            ...
            <div v-if="editingIndex === null && !itemForm.type" class="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <p class="text-xs text-gray-500 text-center">Idéal pour "Énergie & Puissance", "Maison"...</p>
              </button>
              ...
              >
                ...
                <span class="font-bold text-gray-700 group-hover:text-blue-600">Promo (1 Image)</span>
                 <p class="text-xs text-gray-500 text-center">Idéal pour "Essentiels d'Hiver", "Winter Gear"...</p>
              </button>
            </div>
              ...
              <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input v-model="itemForm.title" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500" placeholder="Ex: Énergie & Puissance">
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tag (Condition Météo/Saison)</label>
                    <select v-model="itemForm.tag" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500">
                        <option value="">Par défaut (Toujours)</option>
                        <option value="noel">Noël / Fêtes</option>
                        <option value="rain">Pluie</option>
                        <option value="storm">Vent / Tempête</option>
                        <option value="summer">Été / Chaleur</option>
                        <option value="winter">Hiver / Froid</option>
                        <option value="school">Rentrée scolaire</option>
                    </select>
                  </div>
                </div>
                <div>
                  ...
                  <div class="grid grid-cols-2 gap-2">
                     ...
                     <input v-model="itemForm.link" type="text" class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500" placeholder="URL (ex: /category/power)">
                  </div>
                </div>
              </div>
...
const isUploading = ref(false); // New state
...
const config = ref({
  section: 'weather_picks',
  isActive: true,
  content: {
    mode: 'manual', // 'manual' | 'automatic'
    weatherOverride: '', // Force a specific tag (rain, noel, etc)
...
const itemForm = ref<any>({
  ...
  items: [{}, {}, {}, {}], 
  // Promo specific
  tag: '', // Condition tag (noel, rain, etc)
  subtitle: '',
  promoText: '',
...
const loadConfig = async () => {
  loading.value = true;
  try {
    const response = await PersonalizationService.getSectionConfig('weather_picks');
    if (response.data) {
      config.value = { 
        ...response.data, 
        content: { 
...
const saveConfig = async () => {
  ...
  try {
    ...
    await PersonalizationService.updateSectionConfig('weather_picks', cleanConfig);
    await loadConfig();
    await api.post('/personalization/sections/weather_picks', config.value);
