...
const deviceStatsData = ref<any[]>([]);

// Mapping des couleurs pour les sources
const SOURCE_COLORS: Record<string, string> = {
  'Direct': '#06b6d4',
  'Direct / Inconnu': '#06b6d4',
  'Google': '#8b5cf6',
  'SEO': '#8b5cf6',
  'Social': '#ec4899',
  'WhatsApp': '#22c55e',
...
const waveSeries = computed(() => [
  { name: 'Visites', data: [30, 40, 25, 50, 49, 21, analyticDataReady.value ? 70 : 0] }
]);
...
const analyticDataReady = ref(false);
...
const loadAnalyticsData = async () => {
  try {
    ...
    const [overview, traffic, devices] = await Promise.all([
      statsService.getOverview('30j'),
      statsService.getTrafficSources('30j'),
      statsService.getDeviceStats('30j')
    ]);

    analytics.value = overview;
    
    // Map traffic sources with colors
    trafficSourceData.value = traffic.map((s: any) => ({
        label: s.source,
        val: s.percentage,
        color: SOURCE_COLORS[s.source] || '#94a3b8'
    }));

    deviceStatsData.value = devices;

    // Build deterministic trends from real data
    const ca = (overview as any).chiffreAffaires || 100;
    const cmds = (overview as any).nbCommandes || 10;
    const clients = (overview as any).nbClients || 5;
