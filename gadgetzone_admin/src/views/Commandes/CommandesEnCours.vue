<template>
  <div class="px-2 py-4 sm:p-6 space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-row justify-between items-center gap-4">
      <div>
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Commandes en Cours</h1>
            ...
            <tr v-if="!loading && !error && filteredOrders.length === 0">
              <td colspan="6" class="px-6 py-12 text-center">
                <div class="flex flex-col items-center gap-3">
                  ...
                  <p class="text-sm text-gray-500 dark:text-gray-400">Aucune commande en cours</p>
                </div>
              </td>
            </tr>
          </tbody>
        ...
        <div v-else v-for="order in paginatedOrders" :key="order.id" class="p-3">
          <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-2 min-w-0">
              <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-900/30 flex-shrink-0">
                <span class="text-[10px] font-mono font-bold text-blue-600 dark:text-blue-400">#{{ order.id.toString().padStart(4, '0') }}</span>
              </div>
              <div class="truncate">
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
              ...
              <button 
                v-if="order.status === 'processing'"
                @click="updateOrderStatus(order.id, 'shipped')" 
                class="p-1.5 text-blue-600 bg-blue-50 dark:bg-blue-900/20 rounded-lg"
              >
              ...
              <button 
                v-if="order.status === 'shipped'"
                @click="updateOrderStatus(order.id, 'delivered')" 
                class="p-1.5 text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg"
              >
