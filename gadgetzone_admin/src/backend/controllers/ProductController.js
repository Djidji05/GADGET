...
class ProductController {
    ...
    searchByImage = async (req, res) => {
        try {
            ...
            // 1. Identification par IA
            const ImageRecognitionService = (await import('../services/ImageRecognitionService.js')).default;
            const keywords = await ImageRecognitionService.identify(image);
            console.log('🤖 AI identified keywords:', keywords);

            if (!keywords || keywords.length === 0) {
                return res.json([]);
            }

            // 2. Recherche par les mots-clés trouvés
            ...
            console.error('❌ Controller Error [searchByImage]:', error);
            res.status(500).json({ error: 'Échec de la recherche par image', message: error.message });
        }
    };
}
