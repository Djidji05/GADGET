...
export default class ProductRepository extends BaseRepository {
    ...
    async search(filters = {}, options = {}) {
        ...
        if (q) {
            // Sanitize query to avoid SQL LIKE crashes with backslashes
            const cleanQ = q.replace(/\\/g, '');
            // Typo-tolerant heuristic for SQLite/Postgres fallback:
            // "iphone 15" -> "%iphone%15%" or "%15%iphone%"
            const words = cleanQ.split(' ').filter(word => word.trim().length > 1);
            const fuzzyQ = words.join('%');
            ...
            if (fuzzyQ.length > 2) {
                orConditions.push({ name: { [Op.iLike]: `%${fuzzyQ}%` } });
                orConditions.push({ description: { [Op.iLike]: `%${fuzzyQ}%` } });
                
                // Reverse order %mot2%mot1%
                if (words.length > 1) {
                    const reverseFuzzyQ = [...words].reverse().join('%');
                    orConditions.push({ name: { [Op.iLike]: `%${reverseFuzzyQ}%` } });
                }
            }
