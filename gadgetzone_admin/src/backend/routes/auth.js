...
import { User, Order, SecurityLog } from '../models/index.js';
...
router.post('/verify-2fa', authLimiter, async (req, res) => {
  try {
    ...
    // Reset code
    await user.update({ two_factor_code: null, two_factor_expires: null });

    // Créer le token
    const token = jwt.sign(
      { userId: user.id, email: user.email, role: user.role, lastActivity: Date.now() },
      process.env.JWT_SECRET,
      { expiresIn: process.env.JWT_EXPIRES_IN || '24h' }
    );
 ...
 * Initier la configuration 2FA (envoyer un code)
 */
...
router.post('/2fa/verify-enable', authenticateToken, async (req, res) => {
  try {
    ...
    if (user.two_factor_code !== code || new Date() > user.two_factor_expires) {
 ...
 * Désactiver le 2FA (nécessite mot de passe)
 */
