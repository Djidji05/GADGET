...
/**
 * PUBLIC: /api/stats/track
 * Enregistrer une visite anonyme pour les analyses d'appareils
 */
router.post('/track', async (req, res) => {
  try {
    ...
    // Normalisation du type d'appareil
    let deviceType = result.device.type || 'desktop';
    if (!['mobile', 'tablet', 'desktop', 'console', 'smarttv', 'wearable', 'embedded'].includes(deviceType)) {
        deviceType = 'other';
    }
    ...
    // Échec silencieux pour ne pas impacter l'utilisateur final
    console.error('⚠️ Track Error:', error.message);
    res.status(204).send();
  }
});

// 🔒 SÉCURITÉ: Toutes les autres routes de statistiques nécessitent une authentification admin
...
router.get('/traffic-sources', cacheMiddleware(5), async (req, res) => {
  try {
    ...
    const trafficSources = await sequelize.query(`
      SELECT 
        referrer as source,
        COUNT(*) as visits
      FROM visits
    ...
    const totalVisits = trafficSources.reduce((sum, s) => sum + parseInt(s.visits), 0);
    
    const formattedSources = trafficSources.map(s => ({
      source: s.source || 'Direct / Inconnu',
      visits: parseInt(s.visits),
      percentage: totalVisits > 0 ? parseFloat(((parseInt(s.visits) / totalVisits) * 100).toFixed(1)) : 0
    }));
...
router.get('/device-stats', cacheMiddleware(5), async (req, res) => {
  try {
    ...
    const total = deviceStats.reduce((sum, d) => sum + parseInt(d.count), 0);

    const formatted = deviceStats.map(d => ({
      label: d.device_type.charAt(0).toUpperCase() + d.device_type.slice(1),
      count: parseInt(d.count),
      percentage: total > 0 ? parseFloat(((parseInt(d.count) / total) * 100).toFixed(1)) : 0
    }));

    res.json(formatted);
  } catch (error) {
    console.error('❌ Error in device stats:', error);
    res.status(500).json({ error: 'Erreur lors de la récupération des statistiques d\'appareils' });
  }
});
  ...
  try {
    const alerts = await SecurityLog.findAll({
      where: {
        status: 'unread',
        severity: {
          [sequelize.Sequelize.Op.in]: ['medium', 'high', 'critical']
        }
      },
    ...
    // Total d'alertes non lues pour badger
    const count = await SecurityLog.count({
      where: { status: 'unread' }
    });
    ...
    res.status(500).json({ error: 'Erreur lors de la récupération des alertes de sécurité' });
  }
});

/**
 * PUT /api/stats/security-alerts/:id/resolve
 * Marquer une alerte comme résolue
 */
router.put('/security-alerts/:id/resolve', async (req, res) => {
  try {
    const { id } = req.params;
    const alert = await SecurityLog.findByPk(id);
