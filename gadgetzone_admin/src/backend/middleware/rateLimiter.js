...
export const authLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 5, // Limite stricte de 5 tentatives pour empêcher les attaques par force brute
    message: {
    ...
    handler: async (req, res, next, options) => {
        try {
            // Pour éviter qu'un attaquant ne sature la base de données (DDoS sur le log),
            // nous n'enregistrons l'alerte 'brute_force' QU'UNE SEULE FOIS par fenêtre de blocage.
            // req.rateLimit.current contient le nombre de requêtes.
