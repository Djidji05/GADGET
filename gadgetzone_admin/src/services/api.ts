...
export const statsService = {
  ...
  getTrafficSources: async (period?: string) => {
    const response = await api.get('/stats/traffic-sources', {
      params: { period: period || '30j' }
    });
    return response.data;
  },

  getDeviceStats: async (period?: string) => {
    const response = await api.get('/stats/device-stats', {
      params: { period: period || '30j' }
    });
    return response.data;
  },

  getConversionRate: async (period?: string) => {
    const response = await api.get('/stats/conversion-rate', {
      params: { period: period || '30j' }
    });
    return response.data;
  },
  ...
  getSecurityAlerts: async () => {
    try {
      const response = await api.get('/stats/security-alerts');
      return response.data;
    } catch (error) {
      console.error('⚠️ Security alerts API failed:', error);
      return { alerts: [], count: 0 };
    }
  },

  resolveSecurityAlert: async (id: string) => {
    try {
      const response = await api.put(`/stats/security-alerts/${id}/resolve`);
      return response.data;
    } catch (error) {
      console.error('⚠️ Resolve security alert failed:', error);
      throw error;
    }
  },
